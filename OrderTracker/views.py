...
#Pending Order Reassign
@login_required(login_url="/employee/login/")
def pending_cpo_reassign(request,cpo_id=None):
        context={}
        context['order_tracker'] = 'active'
        u = User.objects.get(username=request.user)
        type = u.profile.type
        ...
        if request.method == 'POST':
                ...
                assign_person = User.objects.get(username=data['assign'])

                cpo.cpo_assign_detail.assign_to = assign_person
                cpo.cpo_assign_detail.save()

                #cpo.cpo_approval_detail.approved_by = request.user
                #cpo.cpo_approval_detail.save()

                vpo = VendorPO.objects.filter(cpo = cpo)
                for item in vpo:
                        item.requester = assign_person
                        item.save()

                vpo_tracker = VendorPOTracker.objects.filter(vpo__cpo=cpo)
                for item in vpo_tracker:
                        item.requester = assign_person
                        item.save()
...
def pending_cpo_lineitems(request,cpo_id=None):
        ...
        if request.method == "GET":
                ...
                context['status'] = pending_cpo.status
